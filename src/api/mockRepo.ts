// src/api/mockRepo.ts
import type {Appointment, Clinic, Doctor, Speciality, ID} from "@/types";
...
export async function getDoctors(opts?: { clinicId?: ID; specialityId?: ID }): Promise<Doctor[]> {
    await lag();
    let res = doctors.slice();
    if (opts?.clinicId) res = res.filter((d) => d.clinicId === opts.clinicId);
    if (opts?.specialityId) res = res.filter((d) => d.specialityId === opts.specialityId);
    return res;
}
...
export async function createAppointment(appt: Omit<Appointment, "id">): Promise<Appointment> {
    ...
    const created: Appointment = {...appt, id: crypto.randomUUID()};
    list.push(created);
